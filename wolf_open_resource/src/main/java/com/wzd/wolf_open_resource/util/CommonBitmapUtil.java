...
public class CommonBitmapUtil {
	...
	public static Bitmap createBitmap(int width, int height) {
		...
		try {
			...
			try {
				...
				Log4JUtil.info("get bitmap oom again");
			}
			return stateBm;
		}
		return stateBm;
	...
	public static Bitmap decodeAssertFile(Context context, String name)
		...
		try {
			...
			Log4JUtil.info("get bitmap oom..." + name);
			System.gc();
			Thread.yield();
			try {
				bm = BitmapFactory.decodeStream(is);
	...
	public static Bitmap decodeResource(Context context, int resId, float scale) {
		...
		try {
			...
			Log4JUtil.info("get bitmap oom..." + resId);
			System.gc();
			Thread.yield();
			try {
	...
	public static Bitmap getBitmapByResIdAndKey(Context context, int resId,
		...
		try {
			...
			Log4JUtil.info("get bitmap oom..." + key);
			System.gc();
			Thread.yield();
			try {
				// 重新构造一个60*60的图片
	...
	public static Bitmap decodeFile(String path, float n) {
		...
		try {
			...
			Log4JUtil.info("get bitmap oom..." + path);
			System.gc();
			Thread.yield();
			try {
				bmp = BitmapFactory.decodeFile(path);
